...
namespace FacebookSystem {
	...
	public:
		...
		ConversationLastMessage(int convID)
		{
			...
			String^ name = FacebookSys::AllAccounts[FacebookSys::user_email]->getmyConversations()[convID]->GetUserEmails()[0];
			LastSenderName->Text = FacebookSys::AllAccounts[name]->GetAccountUserName();
			if (FacebookSys::AllAccounts[FacebookSys::user_email]->getmyConversations()[convID]->GetMessages()[messagesCount - 1].Value == FacebookSys::user_email)
				label1->Visible = true;
